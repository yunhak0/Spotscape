        ...
        for epoch in tqdm(range(self.args.epochs)):
            for batch in self.data_loader:
                ...
                if self.args.lam_ss > 0 and self.n_slices >= 2:
                    cond = batch.node_type[:batch.batch_size][:, None] == batch.node_type[:batch.batch_size][None, :]
                    inside_sim_topk1 = torch.topk((similarity * cond), self.args.sim_k)[0].mean(1)
                    outside_sim_topk1 = torch.topk((similarity * ~cond), self.args.sim_k)[0].mean(1)
